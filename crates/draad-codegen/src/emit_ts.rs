...
use super::types::{is_numeric_rust, TypeCtx};
...
pub(super) fn render_index(
    ...
    // Order: header → optional `custom` import → user surface (types,
    // namespace classes, the `Api` aggregator) → runtime defaults (`Rpc`
    // interface, `RpcError`, `defaultRpc`). TypeScript hoists
    // `type`/`interface`/`class` declarations for type-position
    // references, so the API classes citing `Rpc` and `RpcError` before
    // they're lexically declared is safe. The runtime block sits at the
    // bottom because it's machine-generated noise readers don't need to
    // scroll past to find their own types.
    w.line("// Generated by draad-codegen. Do not edit.");

    if let Some(module) = ctx.custom_module {
        w.blank();
        w.line(&format!("import * as {module} from \"./{module}\";"));
    }

    render_ty_decls(&mut w, ty_items, ctx);
...
pub(super) fn write_index(
    ...
    let out = render_index(ty_items, apis, event_apis, raw_apis, ctx);
...
fn render_ty_decls(w: &mut Writer, ty_items: &[&syn::Item], ctx: &TypeCtx<'_>) {
    for item in ty_items {
        w.blank();
        let rendered = emit_ty_decl(item, ctx);
